...
namespace mk
{
	...
	struct import_directory_entry_t
	{
	...
	static_assert(sizeof(import_directory_entry_t) == 20);
	static_assert(sizeof(import_directory_entry_t) == 0x14);

	bool operator==(import_directory_entry_t const& a, import_directory_entry_t const& b) { return std::memcmp(&a, &b, sizeof(a)) == 0; }

	struct import_hint_name_entry_t
	{
		std::uint16_t m_hint;
		char m_name[2];
	};
	static_assert(sizeof(import_hint_name_entry_t) == 4);
	static_assert(sizeof(import_hint_name_entry_t) == 0x4);
...
namespace mk
{
	...
	typedef void* import_callback_ctx_t;
	typedef void(*import_callback_fn_dll_t)(import_callback_ctx_t const& ctx, char const* const& dll_name, std::uint32_t const& dll_name_len);
	typedef void(*import_callback_fn_import_ordinal_t)(import_callback_ctx_t const& ctx, std::uint16_t const& ordinal);
	typedef void(*import_callback_fn_import_named_t)(import_callback_ctx_t const& ctx, char const* const& import_name, std::uint32_t const& import_name_len);
	struct import_callback_t
	{
		import_callback_ctx_t m_ctx;
		import_callback_fn_dll_t m_fn_dll;
		import_callback_fn_import_ordinal_t m_fn_ordinal;
		import_callback_fn_import_named_t m_fn_named;
	};

	bool process_import_directory_table(import_callback_t const& callback, image_data_directories_t const& image_data_directories, int const& image_data_directories_count, section_headers_t const& section_headers, int const& section_headers_count, mapped_sections_t const& mapped_sections, bool const& is_32_bit);
	bool process_import_address_table(import_callback_t const& callback, section_headers_t const& section_headers, int const& section_headers_count, mapped_sections_t const& mapped_sections, bool const& is_32_bit, std::uint32_t const& import_address_table_rva);
	template<typename uint_t>
	bool process_import_address_table(import_callback_t const& callback, section_headers_t const& section_headers, int const& section_headers_count, mapped_sections_t const& mapped_sections, std::uint32_t const& import_address_table_rva);
	bool process_hint_name_entry(section_headers_t const& section_headers, int const& section_headers_count, mapped_sections_t const& mapped_sections, std::uint32_t const& hint_name_entry_rva, std::uint16_t* const& hint_out, char const** const& name_out, std::uint32_t* const& name_len_out);
...
bool mk::process_import_directory_table(import_callback_t const& callback, image_data_directories_t const& image_data_directories, int const& image_data_directories_count, section_headers_t const& section_headers, int const& section_headers_count, mapped_sections_t const& mapped_sections, bool const& is_32_bit)
{
	if(!(image_data_directories_count >= static_cast<int>(image_data_directory_e::import_table) + 1)) return true;
	std::uint32_t const& imports_rva = image_data_directories[static_cast<int>(image_data_directory_e::import_table)].m_rva;
	std::uint32_t const& imports_size = image_data_directories[static_cast<int>(image_data_directory_e::import_table)].m_size;
	if(!(imports_rva != 0 && imports_size != 0)) return true;
	MK_CHECK_RET(imports_size >= sizeof(import_directory_entry_t), false);
	...
	MK_CHECK_RET(import_directory_table, false);
	...
	std::uint32_t const remaining_elements = remaining_bytes / sizeof(import_directory_entry_t);
	bool found_end = false;
	for(std::uint32_t i = 0; i != remaining_elements; ++i)
	{
		import_directory_entry_t const import_directory_entry = read_binary<import_directory_entry_t>(import_directory_table, i * sizeof(import_directory_entry_t));
		if(import_directory_entry == import_directory_entry_t{})
		{
			found_end = true;
			break;
		}
		...
		MK_CHECK_RET(dll_name_found, false);
		callback.m_fn_dll(callback.m_ctx, dll_name, dll_name_len);

		bool const import_address_table_processed = process_import_address_table(callback, section_headers, section_headers_count, mapped_sections, is_32_bit, import_directory_entry.m_import_lookup_table_rva != 0 ? import_directory_entry.m_import_lookup_table_rva : import_directory_entry.m_import_address_table_rva);
		MK_CHECK_RET(import_address_table_processed, false);
	}
	MK_CHECK_RET(found_end, false);

	return true;
}

bool mk::process_import_address_table(import_callback_t const& callback, section_headers_t const& section_headers, int const& section_headers_count, mapped_sections_t const& mapped_sections, bool const& is_32_bit, std::uint32_t const& import_address_table_rva)
{
	if(is_32_bit)
	{
		return process_import_address_table<std::uint32_t>(callback, section_headers, section_headers_count, mapped_sections, import_address_table_rva);
	}
	else
	{
		return process_import_address_table<std::uint64_t>(callback, section_headers, section_headers_count, mapped_sections, import_address_table_rva);
	}
}

template<typename uint_t>
bool mk::process_import_address_table(import_callback_t const& callback, section_headers_t const& section_headers, int const& section_headers_count, mapped_sections_t const& mapped_sections, std::uint32_t const& import_address_table_rva)
{
	...
	MK_CHECK_RET(import_address_table_, false);
	...
	while(import_address_table_size != elems_end && read_binary<uint_t>(import_address_table, import_address_table_size * sizeof(uint_t)) != uint_t{}) ++import_address_table_size;
	MK_CHECK_RET(import_address_table_size != elems_end && import_address_table_size <= 0xFFFF / 2, false);
	...
	for(std::uint32_t i = 0; i != import_address_table_size; ++i)
	{
		...
		uint_t const value = read_binary<uint_t>(import_address_table, i * sizeof(uint_t));
		bool const flag = (value & s_mask) != 0;
		if(flag)
		{
			uint_t const ordinal_ = value &~ s_mask;
			MK_CHECK_RET(ordinal_ <= 0xFFFFull, false);
			std::uint16_t const ordinal = static_cast<std::uint16_t>(ordinal_);
			callback.m_fn_ordinal(callback.m_ctx, ordinal);
		}
		else
		{
			uint_t const hint_name_table_rva_ = value;
			MK_CHECK_RET(hint_name_table_rva_ <= 0xFFFFFFFFull, false);
			...
			MK_CHECK_RET(hint_name_processed, false);
			callback.m_fn_named(callback.m_ctx, name, name_len);
		}
	}
...
bool mk::process_hint_name_entry(section_headers_t const& section_headers, int const& section_headers_count, mapped_sections_t const& mapped_sections, std::uint32_t const& hint_name_entry_rva, std::uint16_t* const& hint_out, char const** const& name_out, std::uint32_t* const& name_len_out)
{
	int section_idx;
	void const* const hint_name_entry_ = find_in_sections(section_headers, section_headers_count, mapped_sections, hint_name_entry_rva, sizeof(import_hint_name_entry_t), &section_idx);
	MK_CHECK_RET(hint_name_entry_, false);
	std::uint16_t const hint = read_binary<std::uint16_t>(hint_name_entry_);

	char const* const string_begin = static_cast<char const*>(hint_name_entry_) + sizeof(std::uint16_t);
	MK_CHECK_RET(string_begin[0] != '\0', false);
	char const* const string_max = static_cast<char const*>(mapped_sections[section_idx].get_view()) + section_headers[section_idx].m_size_of_raw_data;
	char const* string_end = string_begin + 1;
	while(string_end != string_max && string_end[0] != '\0') ++string_end;
	std::uint32_t const string_len = static_cast<std::uint32_t>(string_end - string_begin);
	MK_CHECK_RET(is_ascii(string_begin, string_len), false);
