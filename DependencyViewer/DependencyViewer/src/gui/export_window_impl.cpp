...
void export_window_impl::destroy_accel_table()
{
	assert(g_accel != nullptr);
...
LRESULT export_window_impl::on_message(UINT const& msg, WPARAM const& wparam, LPARAM const& lparam)
{
	switch(msg)
	{
		...
		break;
		case static_cast<std::uint32_t>(export_window::wm::wm_translateaccelerator):
		{
		...
		break;
		case static_cast<std::uint32_t>(export_window::wm::wm_selectitem):
		{
...
LRESULT export_window_impl::on_wm_translateaccelerator(WPARAM const& wparam, LPARAM const& lparam)
{
	...
	UINT const msg = static_cast<std::uint32_t>(export_window::wm::wm_translateaccelerator);
	LRESULT const ret = DefWindowProcW(m_self, msg, wparam, lparam);
	return ret;
}
...
LRESULT export_window_impl::on_wm_selectitem(WPARAM const& wparam, LPARAM const& lparam)
{
	std::uint16_t const item_idx = static_cast<std::uint16_t>(wparam);
	select_item(item_idx);

	UINT const msg = static_cast<std::uint32_t>(export_window::wm::wm_selectitem);
	LRESULT const ret = DefWindowProcW(m_self, msg, wparam, lparam);
	return ret;
}
...
void export_window_impl::select_item(std::uint16_t const& item_idx)
{
