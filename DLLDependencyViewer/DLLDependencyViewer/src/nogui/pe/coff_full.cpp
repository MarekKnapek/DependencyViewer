...
bool pe_parse_coff_full_32_64(void const* const& fd, int const& file_size, coff_full_32_64 const*& hd)
{
	...
	WARN_M_R(file_size >= static_cast<int>(dosheader.m_pe_offset + (is_32 ? sizeof(coff_full_32) : sizeof(coff_full_64) + dir_cnt * sizeof(data_directory))), L"File too small to contain all directories.", false);
	...
	WARN_M_R(file_size >= static_cast<int>(dosheader.m_pe_offset + (is_32 ? sizeof(coff_full_32) : sizeof(coff_full_64)) + dir_cnt * sizeof(data_directory) + coff_hdr->m_section_count * sizeof(section_header)), L"File too small to contain all section headers.", false);
