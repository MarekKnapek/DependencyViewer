...
bool pe_parse_import_directory_table(void const* const& fd, int const& file_size, import_directory_table& idt)
{
	...
	std::uint32_t const imp_dir_tbl_cnt_max = imp_tbl.m_size / sizeof(import_directory_entry);
	std::uint32_t imp_dir_tbl_cnt = 0xffffffff;
	import_directory_entry const* const imp_dir_tbl = reinterpret_cast<import_directory_entry const*>(file_data + imp_dir_tbl_raw);
	for(std::uint32_t i = 0; i != imp_dir_tbl_cnt_max; ++i)
	{
		import_directory_entry const& imp_dir =imp_dir_tbl[i];
		if
		(
...
bool pe_parse_import_dll_name(void const* const& fd, int const& file_size, import_directory_table const& idt, int const& idx, pe_string& str)
{
	char const* const file_data = static_cast<char const*>(fd);
	import_directory_entry const& imp_dir = idt.m_table[idx];
...
bool pe_parse_import_lookup_table_32(void const* const& fd, int const& file_size, import_directory_table const& idt, int const& idx, import_lookup_table_32& ilt)
{
	char const* const file_data = static_cast<char const*>(fd);
	import_directory_entry const& imp_dir = idt.m_table[idx];
	...
	import_lookup_entry_32 const* const iat = reinterpret_cast<import_lookup_entry_32 const*>(file_data + iat_raw);
...
bool pe_parse_import_lookup_table_64(void const* const& fd, int const& file_size, import_directory_table const& idt, int const& idx, import_lookup_table_64& ilt)
{
	char const* const file_data = static_cast<char const*>(fd);
	import_directory_entry const& imp_dir = idt.m_table[idx];
	std::uint32_t const iat_rva = imp_dir.m_import_lookup_table != 0 ? imp_dir.m_import_lookup_table : imp_dir.m_import_adress_table;
	...
	WARN_M_R(iat_rva >= idt.m_sct->m_virtual_address && iat_rva < idt.m_sct->m_virtual_address + idt.m_sct->m_virtual_size, L"Import address table shall be in the same section as import directory table.", false); // Not sure about this.
	std::uint32_t const iat_raw = idt.m_sct->m_raw_ptr + (iat_rva - idt.m_sct->m_virtual_address);
	import_lookup_entry_64 const* const iat = reinterpret_cast<import_lookup_entry_64 const*>(file_data + iat_raw);
	std::uint32_t const iat_cnt_max = pe_min(64u * 1024u, idt.m_sct->m_raw_ptr + idt.m_sct->m_raw_size - iat_raw);
	std::uint32_t iat_cnt = 0xffffffff;
	for(std::uint32_t i = 0; i != iat_cnt_max; ++i)
	{
...
{
	import_lookup_entry_32 const& ile = ilt.m_table[idx];
...
{
	import_lookup_entry_64 const& ile = ilt.m_table[idx];
...
bool pe_parse_import_lookup_entry_hint_name_32(void const* const& fd, int const& file_size, import_lookup_table_32 const& ilt, int const& idx, hint_name& hntnm)
{
	char const* const file_data = static_cast<char const*>(fd);
	import_lookup_entry_32 const& ile = ilt.m_table[idx];
...
bool pe_parse_import_lookup_entry_hint_name_64(void const* const& fd, int const& file_size, import_lookup_table_64 const& ilt, int const& idx, hint_name& hntnm)
{
	char const* const file_data = static_cast<char const*>(fd);
	import_lookup_entry_64 const& ile = ilt.m_table[idx];
...
bool pe_parse_import_lookup_entry_hint_name(void const* const& fd, int const& file_size, section_header const& sct, std::uint32_t const& hint_name_rva, hint_name& hntnm)
{
	char const* const file_data = static_cast<char const*>(fd);
