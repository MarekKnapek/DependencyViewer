...
main_type process_impl(std::vector<std::wstring> const& file_paths)
{
	...
	for(int i = 0; i != n; ++i)
	{
		...
		string_handle* const dll_names = mo.m_mm.m_alc.allocate_objects<string_handle>(1);
...
void process_r(processor_impl& prcsr)
{
	while(!prcsr.m_queue.empty())
	{
		...
		for(std::uint16_t i = 0; i != n; ++i)
		{
			...
			string_handle const& sub_name = fi.m_import_table.m_dll_names[i];
...
void process_e(processor_impl& prcsr, file_info& fi, file_info& sub_fi, string_handle const& dll_name)
{
	...
	wstring_handle const wstr = file_name_provider::get_correct_file_name(sch.m_mo->m_tmpw.c_str(), static_cast<int>(sch.m_mo->m_tmpw.size()), prcsr.m_mo->m_mm.m_wstrs, prcsr.m_mo->m_mm.m_alc);
...
void pair_imports_with_exports(processor_impl& prcsr, file_info& fi, file_info& sub_fi)
{
	...
	for(int i = 0; i != n; ++i)
	{
		...
		else
		{
			...
			if(hint < enpt.m_count && exp.m_names[enpt.m_enpt[hint]] == name)
			{
			...
			else
			{
				...
				auto const it = std::lower_bound(enpt.m_enpt, enpt_end, name, [&](auto const& e, auto const& v) -> bool { return exp.m_names[e] < v; });
